...
struct ConcurrencyTests {
    ...
    @Test("10 concurrent tasks adding nodes produce zero data corruption (SC-006)")
    ...
    @Test("10 concurrent tasks adding states produce monotonically ordered versions")
    func concurrentStateAdds() async throws {
        ...
        try await withThrowingTaskGroup(of: [TemporalState].self) { group in
            ...
            // All version values should be unique (no two states share a version)
    ...
    @Test("Concurrent edge and node adds do not corrupt referential integrity checks")
    func concurrentEdgeAndNodeAdds() async throws {
        ...
        // Concurrently add edges between existing nodes
        let bounds = TemporalBounds(validFrom: Date(timeIntervalSince1970: 0), validUntil: nil)
        var edgeCount = 0
